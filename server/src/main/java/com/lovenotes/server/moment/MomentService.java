...
public class MomentService {
    ...
    private MediaView mediaView(MediaAssetEntity asset){boolean ready=asset.getStatus()==DomainEnums.MediaStatus.READY;String displayKey=asset.getDisplayStorageKey()==null?asset.getStorageKey():asset.getDisplayStorageKey();String displayContentType=asset.getDisplayStorageKey()==null?asset.getMimeType():"image/webp";return new MediaView(asset.getId(),asset.getKind(),asset.getStatus(),ready?storage.signedGetUrl(displayKey,Duration.ofMinutes(15),displayContentType):null,ready&&asset.getThumbnailStorageKey()!=null?storage.signedGetUrl(asset.getThumbnailStorageKey(),Duration.ofMinutes(15),"image/webp"):null);}
