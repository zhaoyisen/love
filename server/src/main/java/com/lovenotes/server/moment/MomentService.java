...
public class MomentService {
    ...
    public MomentEntity create(UUID actorId,String idempotencyKey,CreateCommand command){
        ...
        validate(command);auditText(command.title(),command.body());UUID coupleId=scope(actorId,command.visibility());List<MediaAssetEntity> media=validateMedia(actorId,command.type(),command.assetIds());
        DomainEnums.MomentStatus status=media.isEmpty()||media.stream().allMatch(asset->asset.getStatus()==DomainEnums.MediaStatus.READY)?DomainEnums.MomentStatus.PUBLISHED:DomainEnums.MomentStatus.UPLOADING;
    ...
    @Transactional
    public MomentEntity update(UUID actorId,UUID id,int version,String title,String body,Instant occurredAt,DomainEnums.Visibility visibility,String mood,List<String> events){MomentEntity moment=requireAuthor(actorId,id);if(moment.getVersion()!=version)throw versionConflict(moment.getVersion());CreateCommand command=new CreateCommand(moment.getType(),title,body,occurredAt,visibility,mood,events==null?List.of():events,List.of());validate(command);auditText(title,body);UUID coupleId=scope(actorId,visibility);moment.update(normalize(title),body,occurredAt,visibility,coupleId);tags.deleteByIdMomentId(id);if(mood!=null&&!mood.isBlank())tags.save(new MomentTagEntity(new MomentTagId(id,DomainEnums.TagType.MOOD,mood)));command.events().forEach(event->tags.save(new MomentTagEntity(new MomentTagId(id,DomainEnums.TagType.EVENT,event))));return moment;}
    ...
    private MomentView view(UUID actorId,MomentEntity m){List<MomentTagEntity> all=tags.findByIdMomentId(m.getId());String mood=all.stream().filter(t->t.getId().getTagType()==DomainEnums.TagType.MOOD).map(t->t.getId().getTagValue()).findFirst().orElse(null);List<String> events=all.stream().filter(t->t.getId().getTagType()==DomainEnums.TagType.EVENT).map(t->t.getId().getTagValue()).toList();List<MediaView> media=assets.findByMomentIdOrderByCreatedAtAsc(m.getId()).stream().map(asset->{boolean ready=asset.getStatus()==DomainEnums.MediaStatus.READY;String displayKey=asset.getDisplayStorageKey()==null?asset.getStorageKey():asset.getDisplayStorageKey();return new MediaView(asset.getId(),asset.getKind(),asset.getStatus(),ready?storage.signedGetUrl(displayKey,Duration.ofMinutes(5)):null,ready&&asset.getThumbnailStorageKey()!=null?storage.signedGetUrl(asset.getThumbnailStorageKey(),Duration.ofMinutes(5)):null);}).toList();return new MomentView(m.getId(),m.getAuthorId(),m.getType(),m.getTitle(),m.getBody(),m.getOccurredAt(),m.getVisibility(),m.getStatus(),m.getVersion(),m.getDeletedAt(),mood,events,media);}
    ...
    public record MomentView(UUID id,UUID authorId,DomainEnums.MomentType type,String title,String body,Instant occurredAt,DomainEnums.Visibility visibility,DomainEnums.MomentStatus status,int version,Instant deletedAt,String mood,List<String> events,List<MediaView> media){}
}
