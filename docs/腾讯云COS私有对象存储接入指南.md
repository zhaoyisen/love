...
display/{userPublicId}/{yyyy}/{mm}/{uuid}.jpg
...
1. 打开“访问管理 CAM → 用户 → 用户列表”。
2. 新建一个仅用于后端申请临时凭证的子用户，例如：

   ```text
   love-notes-cos-sts-dev
   ```

3. 只启用编程访问，不需要控制台登录。
4. 创建自定义策略，仅允许访问目标存储桶和所需操作。
5. 不要授予 `AdministratorAccess`、`QcloudCOSFullAccess` 或 `resource:*`。
6. 为该子用户创建 API 密钥。
7. 将密钥存入服务端 Secret Manager、KMS 或部署环境变量。
8. 不要将密钥写入 Git、配置示例、小程序、日志或聊天记录。
9. 定期轮换密钥，并检查最后使用时间。
...
检查通过后，才将资产状态改为 `UPLOADED`，并投递内容安全和缩略图任务。
...
```http
GET /api/v1/media/{assetId}/access-url
Authorization: Bearer <access-token>
```
...
通过后，返回有效期很短的预签名 URL，例如 5 分钟。时间线优先返回缩略图的短期 URL，只有查看原图时才签发原件 URL。
...
| 前缀/对象 | 建议规则 |
|---|---|
| `tmp/` | 上传会话结束或失败后尽快清理，兜底 1–3 天删除 |
| 未完成分块上传 | 1–3 天自动清理 |
| `deleted/` | 如果采用隔离区，30 天后永久删除 |
...
它会签发仅允许上传单个随机 Key 的 STS 临时凭证，并在 complete 接口读取 COS 对象元数据校验大小。小程序页面当前仍使用本地 mock，需要在后续联调中接入：
...
miniprogram/services/moment-service.ts
miniprogram/stores/upload-store.ts
```
...
- [ ] 验证永久 URL 不能匿名打开；
- [ ] 设置预算告警；
- [ ] 保存 Bucket 和 Region，绝不保存或发送主账号 SecretKey。
